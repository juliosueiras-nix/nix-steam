{ callPackage, gameFileInfo, steamGameFetcher, jq, lib, symlinkJoin, protonWrapperScript, linuxWrapperScript, linkFarm, runCommand }:

{ steamUserInfo, game, gameFiles, drvPath, proton ? null }:

callPackage drvPath {
  realGameLocation = "${steamUserInfo.targetStore}/${game.platform}/${game.name}";
  inherit game gameFileInfo steamGameFetcher steamUserInfo proton protonWrapperScript linuxWrapperScript;
