{ protonWrapperScript, curl, krb5, keyutils, game, proton, lib, steamcmd, steam, writeText, steam-run, writeScript, writeScriptBin, gameFiles, lndir, steamUserInfo, realGameLocation, ... }:
...
writeScriptBin game.name ''
  ...
  ${steam-run}/bin/steam-run ${writeScript "fix-${game.name}" ''
    ...
    $PROTON_HOME/proton waitforexitandrun ./ProjectWingman.exe -nohmd
  ''}
''
